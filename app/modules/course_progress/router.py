...
async def _list_students_for_course(db: AsyncSession, course_id: uuid.UUID) -> list[StudentProgressRead]:
...
async def list_course_students_endpoint(
    ...
    return await _list_students_for_course(db, course_id)


@faculty_router.get("/courses/{course_id}/students", response_model=list[StudentProgressRead])
async def list_faculty_course_students_endpoint(
    course: Course = Depends(require_course_faculty()),
    db: AsyncSession = Depends(get_db),
) -> list[StudentProgressRead]:
    return await _list_students_for_course(db, course.id)
